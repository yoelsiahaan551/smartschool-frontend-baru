...
// STORAGE
// =========================================================
const STORAGE_KEY = "sarpras_gedung_data";

const getDefaultData = () => [
  { id: 1, nama: "Gedung A", kode: "A", status: "aktif", deskripsi: "Gedung utama perkantoran dan ruang teori" },
  { id: 2, nama: "Gedung B", kode: "B", status: "aktif", deskripsi: "Gedung laboratorium dan praktikum" },
  { id: 3, nama: "Gedung C", kode: "C", status: "aktif", deskripsi: "Gedung perpustakaan dan aula" },
  { id: 4, nama: "Gedung D", kode: "D", status: "nonaktif", deskripsi: "Gedung serba guna (renovasi)" },
  { id: 5, nama: "Gedung E", kode: "E", status: "aktif", deskripsi: "Gedung workshop dan ruang kreatif" },
  { id: 6, nama: "Gedung F", kode: "F", status: "aktif", deskripsi: "Gedung olahraga dan kegiatan ekstrakurikuler" },
];
...
export default function SarprasGedungPage() {
  ...
  // Simulasi jumlah lantai per gedung
  const getJumlahLantai = (gedungId) => {
    return Math.floor((gedungId * 7) % 5) + 1;
  };
  ...
  const handleDelete = (id, nama) => {
    if (!window.confirm(`Yakin ingin menghapus gedung "${nama}"?`)) return;
  ...
  const filtered = data.filter((item) => {
    ...
    const match = item.nama.toLowerCase().includes(keyword) || 
  ...
  // Export
  const exportCSV = () => {
    const headers = ["No", "Nama Gedung", "Kode", "Status", "Deskripsi"];
    const rows = data.map((item, idx) => [idx + 1, item.nama, item.kode, item.status, item.deskripsi || ""]);
    const csv = [headers.join(","), ...rows.map((row) => row.join(","))].join("\n");
    const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = `data_gedung_sarpras_${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  const exportExcel = () => {
    const headers = ["No", "Nama Gedung", "Kode", "Status", "Deskripsi"];
    let html = `<html><head><meta charset="UTF-8"><style>th,td{border:1px solid #ccc;padding:6px 10px;font-size:12px;font-family:Arial,sans-serif;} th{background:#f0f0f0;font-weight:bold;}</style></head><body><table><tr>${headers.map(h => `<th>${h}</th>`).join("")}</tr>`;
    data.forEach((item, idx) => {
      html += `<tr><td>${idx + 1}</td><td>${item.nama}</td><td>${item.kode}</td><td>${item.status}</td><td>${item.deskripsi || ""}</td></tr>`;
    });
    html += `</table></body></html>`;
    const blob = new Blob([html], { type: "application/vnd.ms-excel" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = `data_gedung_sarpras_${new Date().toISOString().slice(0, 10)}.xls`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(link.href);
            ...
            <div className="mx-auto w-full max-w-[1600px] space-y-4 sm:space-y-5 lg:space-y-6">
                ...
                <div className="relative flex flex-col gap-4 p-5 sm:p-6 lg:flex-row lg:items-center lg:justify-between lg:px-8 lg:py-6">
                  <div className="flex min-w-0 items-start gap-3 sm:gap-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-1.5 sm:gap-2">
                        <Layers size={13} className="shrink-0 text-blue-400 sm:h-[14px] sm:w-[14px]" strokeWidth={2} />
                        <p className="text-xs leading-5 text-slate-500 sm:text-sm">
                          Kelola data gedung untuk sarana dan prasarana sekolah.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex w-full flex-wrap gap-2 sm:flex-row lg:w-auto">
                    ...
                    <div className="relative">
                      ...
                      >
                        <Download size={16} className="sm:h-[17px] sm:w-[17px]" />
                        Export
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                ...
                <StatCard
                  ...
                  label="Gedung Aktif"
                  value={totalAktif}
                  description="Gedung yang aktif"
                  iconClass="bg-emerald-50 text-emerald-600"
                  valueClass="text-emerald-700"
                />
                <StatCard
                  icon={XCircle}
                  label="Gedung Nonaktif"
                  value={data.length - totalAktif}
                  description="Gedung nonaktif"
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_2px_10px_rgba(15,23,42,0.04)] sm:p-5">
                <div className="mb-4 flex items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Cari dan filter data gedung</p>
                  </div>
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
                  ...
                  <button
                    onClick={() => { setSearch(""); setFilterStatus("semua"); setSortBy("nama_asc"); }}
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3">
                  <p className="text-xs text-slate-400">
                    Menampilkan <span className="font-semibold text-slate-600">{totalItems}</span> data gedung
                  </p>
                </div>
              </section>
              ...
              <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_3px_14px_rgba(15,23,42,0.05)]">
                <div className="border-b border-slate-100 px-5 py-4 sm:px-6 sm:py-5">
                  <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <h2 className="text-sm font-semibold text-slate-800">Daftar Gedung</h2>
                      <p className="text-xs text-slate-500">Data master gedung untuk sarana dan prasarana</p>
                    </div>
                    <div className="text-xs text-slate-500">{totalItems} data</div>
                  </div>
                </div>
                  ...
                  <table className="w-full min-w-[800px] border-collapse">
                    <thead>
                      <tr className="border-b border-slate-200 bg-slate-50/80">
                        <th className="w-14 px-4 py-3 text-center text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">No</th>
                        <th className="min-w-[180px] px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">Nama Gedung</th>
                        <th className="w-24 px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">Kode</th>
                        <th className="w-28 px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">Lantai</th>
                        <th className="min-w-[200px] px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">Deskripsi</th>
                        <th className="w-28 px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">Status</th>
                          ...
                          <tr key={item.id} className="transition-colors hover:bg-slate-50/70">
                            ...
                            <td className="px-4 py-3.5">
                              <div className="flex items-center gap-1.5 text-sm text-slate-600">
                                ...
                                {getJumlahLantai(item.id)} Lantai
                              </div>
                            </td>
                            <td className="px-4 py-3.5">
                              <p className="truncate text-sm text-slate-500">{item.deskripsi || "-"}</p>
                            </td>
                            <td className="px-4 py-3.5">
                              <span
                ...
                {currentItems.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <h3 className="mt-4 text-sm font-semibold text-slate-700">Tidak ada data</h3>
                    <p className="mt-1 text-xs text-slate-400">
                      {search ? "Tidak ditemukan data yang sesuai dengan pencarian." : "Belum ada data gedung."}
                    </p>
                  </div>
                )}
              ...
              <footer className="border-t border-slate-200/70 pt-4 text-center sm:pt-5">
                <p className="text-xs text-slate-400">
                  © 2026 SmartSchool • Pengelolaan Gedung - Sarana & Prasarana
                </p>
              </footer>
            </div>
          </div>
