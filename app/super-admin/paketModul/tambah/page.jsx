...
export default function TambahPaketPage() {
  ...
  const isDuplikat = searchParams.get("duplikat") === "true";
  ...
  useEffect(() => {
    async function loadFitur() {
      try {
        ...
        setFiturList(data.map((item, index) => normalizeFeature(item, index)));
      } catch (err) {
        console.error("Gagal memuat fitur:", err);
        setError(err?.response?.data?.message || err?.message || "Gagal memuat daftar fitur.");
      } finally {
  ...
  useEffect(() => {
    ...
    try {
      const data = sessionStorage.getItem("duplikatPaket");
      if (!data) return;
      const parsed = JSON.parse(data);
      setNama(parsed.nama || "");
      setDeskripsi(parsed.deskripsi || "");
      setHarga(parsed.harga || 0);
      setSiklus(parsed.siklus || "bulan");
      setStatus(parsed.status || "aktif");
      setFiturTerpilih(parsed.fiturIds || []);
      sessionStorage.removeItem("duplikatPaket");
    } catch (err) {
      console.error("Gagal membaca data duplikat:", err);
    }
  }, [isDuplikat]);
  ...
  function toggleFitur(id) {
    setFiturTerpilih((current) => {
      ...
      if (exists) return current.filter((item) => String(item) !== String(id));
      return [...current, id];
    });
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err?.response?.data?.message || err?.response?.data?.error || err?.message || "Gagal menyimpan paket.");
    } finally {
      setSaving(false);
    }
  }
  ...
  const selectedFeatures = useMemo(() => {
    return fiturList.filter((fitur) =>
      fiturTerpilih.some((id) => String(id) === String(fitur.id))
    );
  }, [fiturList, fiturTerpilih]);

  return (
    // Hapus overflow-x-auto, biarkan flex normal tanpa scroll horizontal
    <div className="flex min-h-screen bg-[#f8fafc]">
      {/* Sidebar – tanpa flex-shrink-0, biarkan mengikuti aliran */}
      <Sidebar
        active={activeMenu}
      ...
      {/* Konten utama – flex-1 dan min-w-0 agar bisa menyusut */}
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <main className="flex-1">
          {/* Container utama tanpa max-width, lebar penuh, padding tetap */}
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-800">
                        {isDuplikat ? "Duplikat Paket" : "Tambah Paket Baru"}
                      </h1>
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 border border-blue-100 px-2.5 py-1 text-xs font-semibold text-blue-600">
                        <Sparkles size={12} /> Paket Langganan
                      </span>
                    </div>
            ...
            {error && (
              <div className="mb-6 flex items-start gap-3 rounded-xl border border-rose-200 bg-rose-50 p-4 shadow-sm">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-rose-700">Terjadi kesalahan</p>
                  <p className="mt-0.5 text-sm leading-5 text-rose-600">{error}</p>
                </div>
                <button onClick={() => setError("")} className="shrink-0 rounded-lg p-1.5 text-rose-400 hover:bg-rose-100 hover:text-rose-600">
                  <X size={16} />
                </button>
              </div>
            )}
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_4px_20px_rgba(15,23,42,0.04)]">
                  ...
                  <div className="border-b border-slate-100 px-5 py-5 sm:px-7">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="text-base sm:text-lg font-bold text-slate-800">Informasi Paket</h2>
                        <p className="mt-0.5 text-xs sm:text-sm text-slate-400">Lengkapi detail paket sebelum disimpan.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6 p-5 sm:p-7">
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-slate-700">
                        Nama Paket <span className="ml-1 text-rose-500">*</span>
                      </label>
                      <p className="mt-1 text-xs sm:text-sm text-slate-400">Nama yang akan ditampilkan kepada sekolah.</p>
                      <input
                        value={nama}
                    ...
                    {/* Deskripsi */}
                    <div>
                      <label className="block text-sm font-semibold text-slate-700">Deskripsi Paket</label>
                      <p className="mt-1 text-xs sm:text-sm text-slate-400">Jelaskan secara singkat manfaat paket ini.</p>
                      <textarea
                        value={deskripsi}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div>
                        <label className="block text-sm font-semibold text-slate-700">Harga Paket</label>
                        <p className="mt-1 text-xs sm:text-sm text-slate-400">Masukkan harga dalam Rupiah.</p>
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-slate-700">Siklus Pembayaran</label>
                        <p className="mt-1 text-xs sm:text-sm text-slate-400">Tentukan periode pembayaran.</p>
                        <div className="relative mt-3">
                          <CalendarDays size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                          <select
                            value={siklus}
                    ...
                    {/* Status */}
                    <div>
                      <label className="block text-sm font-semibold text-slate-700">Status Paket</label>
                      <p className="mt-1 text-xs sm:text-sm text-slate-400">Tentukan apakah paket dapat digunakan oleh sekolah.</p>
                      <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        >
                          <span
                            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${
                              status === "aktif" ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-400"
                            }`}
                          >
                            <CircleCheck size={18} />
                          </span>
                          <span className="min-w-0">
                            <span className="block text-sm font-semibold text-slate-700">Aktif</span>
                            <span className="block text-xs text-slate-400">Paket tersedia untuk sekolah</span>
                          </span>
                          {status === "aktif" && <Check size={16} className="ml-auto shrink-0 text-emerald-500" />}
                        </button>
                        <button
                          type="button"
                        ...
                        >
                          <span
                            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${
                              status === "nonaktif" ? "bg-slate-600 text-white" : "bg-slate-100 text-slate-400"
                            }`}
                          >
                            <ShieldCheck size={18} />
                          </span>
                          <span className="min-w-0">
                            <span className="block text-sm font-semibold text-slate-700">Nonaktif</span>
                            <span className="block text-xs text-slate-400">Paket tidak tersedia</span>
                          </span>
                          {status === "nonaktif" && <Check size={16} className="ml-auto shrink-0 text-slate-600" />}
                        </button>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
                        <div>
                          <label className="block text-sm font-semibold text-slate-700">Modul / Fitur</label>
                          <p className="mt-1 text-xs sm:text-sm text-slate-400">Pilih modul yang termasuk dalam paket.</p>
                        </div>
                        <div className="inline-flex w-fit items-center gap-1.5 rounded-full bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-600">
                          <Check size={13} /> {fiturTerpilih.length} dipilih
                        </div>
                      </div>
                      ...
                      {loadingFitur ? (
                        <div className="mt-4 flex min-h-[180px] items-center justify-center rounded-xl border border-slate-200 bg-slate-50/50">
                          <div className="flex flex-col items-center gap-3">
                            ...
                            <p className="text-sm text-slate-400">Memuat daftar modul...</p>
                          </div>
                        </div>
                      ) : (
                        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {fiturList.map((fitur) => {
                            const checked = fiturTerpilih.some((id) => String(id) === String(fitur.id));
                              ...
                              >
                                <span
                                  className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md border transition-all ${
                                    checked ? "border-blue-600 bg-blue-600" : "border-slate-300 bg-white group-hover:border-blue-300"
                                  }`}
                                >
                                ...
                                <span
                                  className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${
                                    checked ? "bg-white text-blue-600 shadow-sm" : "bg-slate-100 text-slate-400"
                                  }`}
                                >
                                  <Icon size={16} />
                                </span>
                                <span className="min-w-0 flex-1">
                                  <span className={`block truncate text-sm font-semibold ${checked ? "text-blue-700" : "text-slate-700"}`}>
                                    {fitur.nama}
                                  </span>
                                  {fitur.deskripsi && (
                                    <span className="mt-0.5 block truncate text-xs text-slate-400">{fitur.deskripsi}</span>
                                  )}
                                </span>
                              </button>
                            );
                          ...
                          {fiturList.length === 0 && (
                            <div className="sm:col-span-2 rounded-xl border border-dashed border-slate-200 bg-slate-50 py-10 text-center">
                              ...
                              <p className="text-sm font-medium text-slate-500">Belum ada modul</p>
                              <p className="mt-1 text-xs text-slate-400">Data fitur belum tersedia dari server.</p>
                            </div>
                          )}
                        </div>
                      )}
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_4px_20px_rgba(15,23,42,0.04)]">
                  <div className="bg-gradient-to-br from-blue-600 to-blue-700 px-5 py-5 text-white">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-[11px] font-semibold uppercase tracking-wider text-blue-100">Preview</p>
                        <h2 className="mt-1 text-lg font-bold">Paket Sekolah</h2>
                      </div>
                  ...
                  <div className="p-5">
                    <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="text-xs font-medium text-slate-400">Nama Paket</p>
                          <h3 className="mt-1 break-words text-lg font-bold text-slate-800">{nama.trim() || "Nama Paket"}</h3>
                        </div>
                        <span
                      ...
                      <p className="mt-3 min-h-[48px] text-xs leading-5 text-slate-400">
                        {deskripsi.trim() || "Deskripsi paket akan tampil di sini."}
                      </p>
                      <div className="mt-4 border-t border-slate-200 pt-4">
                        <p className="text-[11px] font-medium text-slate-400">Harga</p>
                        <div className="mt-1 flex flex-wrap items-baseline gap-1">
                          <span className="text-xl font-bold text-slate-800">{formatRupiah(harga)}</span>
                          {Number(harga) > 0 && <span className="text-xs text-slate-400">/ {siklus}</span>}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-5">
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-bold text-slate-700">Modul Termasuk</p>
                        <span className="text-xs font-semibold text-blue-600">{selectedFeatures.length} fitur</span>
                      </div>
                      <div className="mt-3 space-y-2">
                        {selectedFeatures.slice(0, 6).map((fitur) => {
                          const Icon = fitur.icon || Layers;
                          return (
                            <div key={fitur.id} className="flex items-center gap-2.5 rounded-lg border border-slate-100 bg-white px-3 py-2.5">
                              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-blue-600">
                                <Icon size={14} />
                              </span>
                              <span className="min-w-0 flex-1 truncate text-xs font-medium text-slate-600">{fitur.nama}</span>
                        ...
                        {selectedFeatures.length === 0 && (
                          <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 py-7 text-center">
                            ...
                            <p className="text-xs font-medium text-slate-500">Belum ada fitur dipilih</p>
                            <p className="mt-1 text-[11px] text-slate-400">Pilih modul dari form.</p>
                          </div>
                        )}
                        {selectedFeatures.length > 6 && (
                          <p className="pt-1 text-center text-[11px] font-medium text-slate-400">
                            + {selectedFeatures.length - 6} fitur lainnya
                          </p>
                        )}
                      </div>
                    </div>
